...
def remote_tip(remote_url: str, ref: str, deadline: Deadline) -> str | None:
    out = _git(["ls-remote", remote_url, ref], deadline)
    return out.split()[0] if out.strip() else None


def ensure_clone(remote_url: str, work_dir: Path, deadline: Deadline) -> Path:
    """Clone once, fetch afterwards; the clone lives under the private state dir."""
    if (work_dir / ".git").is_dir():
        _git(["fetch", "origin", "--prune"], deadline, cwd=work_dir)
    else:
        work_dir.parent.mkdir(parents=True, exist_ok=True)
        _git(["clone", "--quiet", remote_url, str(work_dir)], deadline)
    return work_dir


def checkout_new(work_dir: Path, branch: str, base_ref: str, deadline: Deadline) -> None:
    _git(["checkout", "--quiet", "-B", branch, base_ref], deadline, cwd=work_dir)


def checkout_existing(work_dir: Path, branch: str, deadline: Deadline) -> bool:
...
def stage_and_commit(
    ...
    if diff.code == 0:
        return None
    _commit(work_dir, message, deadline)
    return current_head(work_dir, deadline)


def _commit(work_dir: Path, message: str, deadline: Deadline) -> None:
...
def current_head(work_dir: Path, deadline: Deadline) -> str:
    return _git(["rev-parse", "HEAD"], deadline, cwd=work_dir).strip()


def fetch_pr_head(work_dir: Path, number: int, head_ref: str, deadline: Deadline) -> str:
...
def ls_tree(work_dir: Path, commit: str, deadline: Deadline) -> dict[str, str]:
    """Map path -> git mode for one commit; catches symlinks/exec/submodules."""
    out = _git(["ls-tree", "-r", commit], deadline, cwd=work_dir)
    modes: dict[str, str] = {}
...
def show_file(work_dir: Path, commit: str, path: str, deadline: Deadline) -> str | None:
...
def push_branch(work_dir: Path, branch: str, deadline: Deadline) -> None:
